...
- **Контекстне меню файлу** у file-explorer-і Obsidian (через
  ...
  - `Show history` → відкриває diff2 view у History list для цього path
    (R2.3).
- **Command palette**:
  - `Diff2: Compare two files…` — picker для обох сторін.
  - `Diff2: Compare active file with…` — active як top, picker для bottom.
  - `Diff2: Show history of active file` — History list для active file.
...
- Через command palette ("Diff2: Open Diff-Edit for this conflict")
...
**R7.7. Undo/commit model — Obsidian-style.**
...
- **`Ctrl+Z` / `Cmd+Z`** працює стандартно — відкочує одну дію за
  ...
  `Cmd+Shift+Z` — redo.
- **Зміни лишаються у буфері**, не пишуться у vault постійно. Це
...
**`[←]` back-arrow — точка коміту**. Натискання `[←]`:
1. Записує поточний стан document buffer у vault (заміщає ours).
2. CM6-історія анулюється — після виходу Ctrl+Z більше не діє.
3. Закриває detail view, повертає у list view (R2.2).
4. Vault-event `modify` спрацьовує → T2 trigger (R4.1) перевіряє
   SHA(ours) === SHA(theirs sibling):
   - **Match** → auto-resolve: sibling видаляється, ConflictStore
     запис очищується, конфлікт зникає зі списку. Це "всі chunk-и
     розв'язані" сценарій — користувач довів файл до повної
     еквівалентності theirs.
   - **No match** → конфлікт лишається у списку, але з прогресом
     (буфер ours тепер ближче до theirs). Наступного разу при
     відкритті diff покаже менше divergent chunks (тільки ті, що
     лишились).

Це **повністю узгоджується з Obsidian-філософією "no manual save"**:
користувач не натискає окрему кнопку `[Save]` — saving відбувається
як побічний ефект навігації назад (`[←]`). Користувач думає лише
про "я закінчив роботу з цим конфліктом", не про "де натиснути
save".

**Дві відмінні дії "виходу" з detail view**:

- **`[←]` back arrow** (у toolbar) — **save + повернення у list
  view** в тому ж tab-і. Це експліцитне "я закінчив редагування цього
  конфлікту, хочу зберегти роботу і повернутись до списку". Сесія
  завершилась успішно, tab лишається відкритим.
- **Native tab close** (X у tab-header / `Cmd+W` / `Ctrl+W`) —
  **discard buffer**, закрити tab повністю. Це інтуїтивно
  "закриваю вікно — викидаю усе, що там було". Робота поточної сесії
  втрачається. Re-opening diff2 tab → list view → клік по тому ж
  конфлікту → **нова чиста сесія**, `initialOursSnapshot` = ours-on-disk
  (тобто стан після **попереднього** успішного `[←]`, не той, що був
  у щойно закритому tab-і).

Жодних confirm-діалогів на tab-close — це матчить Obsidian-style "no
nag prompts". Користувач, що хоче зберегти, явно клікає `[←]` перед
закриттям tab-у.

**Tab switching (Cmd+Tab у межах Obsidian)** не ламає сесію — leaf
лишається живим у background, buffer переживає. Тільки **явне
закриття tab-у** знищує буфер.

Recovery-сценарії:
- "Хочу зберегти і продовжити пізніше" → `[←]` → tab можна закривати.
- "Все, я наплутав, скасуйте усе" → close tab → re-open diff2 →
  той самий конфлікт чекає.
- "Зробив пів-роботи, хочу відкласти" → `[←]` зараз (save partial,
  R4 T2 виявить, що file still divergent, конфлікт лишається у
  списку з прогресом).

**Жодної окремої `[Discard changes]` кнопки** у toolbar немає. CM6
default `history` extension підтримує ~100 transactions глибини, що з
запасом покриває realistic DiffPane-сесію. Користувач, який хоче
відмінити багато дій під час сесії, тримає `Ctrl+Z` (знайомий шаблон,
не потребує окремого UI). Для "discard all and exit" — закрити tab.

**Після `[←]` (або close) un-resolve неможливий**: sibling видалений,
`.conflicts/<id>/` видалений (якщо auto-resolve спрацював).
Recovery шляхи:
- Ручне редагування файлу у vault (звичайним Obsidian editor).
- Наступний sync принесе remote-версію → новий sibling створиться,
  якщо ours все ще divergent від remote.

Це обмеження — навмисне. Альтернатива (зберігати "undo stash" з
sibling-ом N хвилин) — додаткова complexity без відповідного
benefit-у, бо CM6 Ctrl+Z вже покриває 95% "ой я не те клікнув"
випадків поки користувач у редакторі.
...
- [ ] DiffPane у Compare-режимі (R7.9c toolbar, default Reference,
  ...
  widgets)
- [ ] Тест A23: контекстне меню → Compare with… → fuzzy picker → дві
  md-файли відкриваються у DiffPane
- [ ] Тест A24 (desktop): Browse filesystem → обрати файл з-поза
  vault → DiffPane показує його як theirs з `fs://` заголовком
- [ ] Тест A25 (mobile): кнопка `[Browse filesystem…]` НЕ
...
- [ ] Реалізувати block-widget marker decorations для
  `<<< deviceName: [apply][remove]`, `=== [apply both][remove both]`,
  `>>> deviceName: [apply][remove]` з функціональними кнопками.
...
- [ ] Action-кнопки: семантика `apply`/`remove` для верхнього /
  нижнього блоків і `apply both`/`remove both` для середнього (R7.5).
