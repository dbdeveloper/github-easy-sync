...
- **Etap 7**: not started, but partially anticipated — `src/sync2/gitignore-invariants.ts` is already extracted and called from `Sync2Manager` (start of `syncAll`, start of `syncFile` for paths under `<configDir>/`, and `notePathSelfWritten` after a successful push of one of the two managed gitignores). `experimentalSync2` still defaults to `false`; legacy modules still resident; `src/sync2/*` not moved up to `src/`.
- **Plugin rename `github-gitless-sync` → `github-easy-sync`**: partial. Manifest filename, log filename, invariant block, and commit-message defaults already use the new name. Still using the old name: `manifest.json` `id`, repo directory, README. Finish this before or together with Etap 7 cutover.
...
| 7. Cutover (incl. plugin rename) | ~200 (deletions + rename) | ~80 (cleanup) | 1–1.5 days |
| **Total remaining** | **~200** | **~80** | **~1.5 days** |
