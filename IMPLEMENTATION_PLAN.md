...
3. **Мінімізувати дотики за межами Diff-Edit.** За замовчуванням
   нові файли під `src/sync2/views/` + `src/sync2/trash-store.ts` +
   `src/sync2/diff-edit-events.ts` + `src/sync2/external-diff.ts` +
...
Реалізація: малий event-emitter `DiffEditEvents` у `src/sync2/`:
...
- `src/sync2/trash-store.ts` — `TrashStore` (analogue до
  `ConflictStore`): `load()`, `create({path, content/move})`,
  `restore(id)`, `confirmDeleted(paths[])`, `list()`, `clearAll()`.
  In-memory indexes `byId`, `byOriginalPath`.
- `src/sync2/diff-edit-events.ts` — невеликий event-emitter
  (типобезпечний `EventTarget` wrapper). 30–50 рядків.
- `src/sync2/views/diff-edit-view.ts` — нова ItemView, що замінить
  `ConflictView`. View type id — `"sync2-diff-edit"`. Форма UI див.
  TBD-розділ.
- `src/sync2/views/sync-summary-modal.ts` — нова summary-модалка
  з тексту R1.2 і двома кнопками.
- `src/sync2/views/history-list.ts` — побудова списку commit-ів
  для path-а (GitHub + push-queue fallback).
- `src/sync2/views/deleted-list.ts` — побудова уніфікованого
  списку для "Recently deleted" (trash + GitHub-only).
- `src/sync2/external-diff.ts` — argv-splitter + child_process
...
- `src/sync2/views/conflict-modal.ts` — **видалити**
  (разом з тестом).
- `src/sync2/views/conflict-view.ts` — **переіменувати у diff-edit-view.ts**
  або зберегти shim що ре-експортує DiffEditView як ConflictView
  для backwards-сумісності внутрішніх тестів (вирішується при
  імплементації).
- `src/sync2/conflict-store.ts` — додати emit викликів подій;
  додати метод `findEquivalentSibling(ours, vaultPath)` для T2-перевірки.
...
- `src/sync2/sync2-manager.ts` — у `applyRemoteDeletion`
  гарантовано НЕ створювати trash entry; у `processBatch` після
  успішного commit викликати `trashStore.confirmDeleted(batch.deletions)`
  через deps.
- `src/main.ts` —
  - видалити state `suppressConflictModals`, `openConflictViewAfterSync`,
    `resolveNowPath`
  - переробити `handleSync2Conflict` під R1.1 (без модалки, безумовний
    deferred return)
  - після `drain()` показати summary modal (R1.2)
  - підписатись на `vault.on('delete')` → `trashStore.create()` +
    move file. На `vault.on('modify' | 'create')` → перевірка
    auto-resolve T2 (R4.2) + rename-deduplication (R3.3).
  - зареєструвати новий view type `sync2-diff-edit`, прибрати
    `sync2-conflict-view` (або зберегти як alias-redirect, щоб
    відкриття старих linked workspaces не падало).
...
- [ ] `src/sync2/diff-edit-events.ts` + unit тести
...
- [ ] `src/sync2/external-diff.ts` — argv-splitter (з повагою до
...
- [ ] Replace `sync2-conflict-view` registration на
  `sync2-diff-edit`, додати alias-redirect.
