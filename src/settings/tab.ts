...
export default class GitHubSyncSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    // ── Stage 6 diagnostic — TEMPORARY ──────────────────────────────
    // Validates whether `fetch()` inside a Web Worker can reach the
    // GitHub REST API on the current platform (Capacitor on
    // mobile, Electron on desktop). The result decides whether
    // Stage 6's "GitHub HTTP in worker" migration proceeds as
    // planned or falls back to "push-side SHA-first only".
    // Remove this button before the Stage 6 final commit.
    new Setting(containerEl)
      .setName("Stage 6: Worker fetch CORS test")
      .setDesc(
        "Spawns a Web Worker via Blob URL, has the worker call " +
          "GET /repos/{owner}/{repo} via native `fetch`, and reports " +
          "status + body length + any CORS / fetch error back. " +
          "If this succeeds, full network worker migration is viable; " +
          "if it fails on mobile, we fall back to keeping HTTP on " +
          "the main thread. Uses the configured token + repo settings.",
      )
      .addButton((button) => {
        button.setButtonText("Run CORS test").onClick(async () => {
          await runWorkerFetchCorsTest(this.plugin);
        });
      });
...
// Stage 6 diagnostic — TEMPORARY. Spawns a Worker via inline Blob
// URL, has it call `fetch` against the GitHub REST API, and reports
// whether the call succeeds.  We need to validate that Web Workers
// inside Capacitor's WebView (Android / iOS) can make cross-origin
// HTTPS requests to api.github.com before committing to the Stage 6
// "GitHub HTTP in worker" migration. Obsidian itself uses
// `requestUrl` because the desktop Electron environment bypasses
// CORS for plugin code — that workaround isn't available in worker
// scope, where `fetch` is the only HTTP API and CORS rules apply.
//
// Test method: GET /repos/{owner}/{repo} with Authorization header.
// The endpoint requires auth (private repo) but the request itself
// is a single GET — keeps the test minimal. Any CORS preflight
// failure surfaces immediately as a fetch error.
async function runWorkerFetchCorsTest(
  plugin: GitHubSyncPlugin,
): Promise<void> {
  ...
  if (
    ...
    new Notice(
      "Configure GitHub token + owner + repo before running the CORS test.",
      8000,
    );
    return;
  }

  // Worker source as a JS string. Uses native fetch which on
  // Capacitor goes through the WebView's network stack. Reports
  // success/failure shape back to main thread; main thread surfaces
  // result via Notice + plugin log.
  const workerSource = `
    self.onmessage = async (e) => {
      const { id, url, token } = e.data;
      const t0 = Date.now();
      try {
        const resp = await fetch(url, {
          ...
          headers: {
            ...
            "Accept": "application/vnd.github+json",
            "X-GitHub-Api-Version": "2022-11-28",
            "User-Agent": "github-easy-sync-cors-test",
          },
        });
  ...
  try {
    ...
    plugin.logger.error("[cors-test] Worker constructor threw", {
      error: String(err),
    });
    new Notice(
      `Worker construction failed: ${err}\nCORS test cannot proceed.`,
      15000,
    );
    return;
  }
  ...
  new Notice(
    "Worker spawned, dispatching fetch to api.github.com…",
    5000,
  );

  // Race the worker against a 20 s timeout so a hung fetch doesn't
  // leave the test running forever.
  const result = await new Promise<{
    ...
    const timer = setTimeout(() => {
      resolve({ ok: false, timeout: true, error: "Worker timeout 20s" });
    }, 20000);
    worker!.onmessage = (e) => {
      clearTimeout(timer);
      resolve(e.data);
    ...
    worker!.postMessage({
      id: "cors-test",
      url: `https://api.github.com/repos/${settings.githubOwner}/${settings.githubRepo}`,
  ...
  plugin.logger.info("[cors-test] result", result);

  if (result.ok) {
    new Notice(
      `🎉 Worker fetch SUCCESS\n` +
        `status: ${result.status}\n` +
        `body: ${result.bodyChars} chars\n` +
        `content-type: ${result.contentType}\n` +
        `duration: ${result.durationMs} ms\n\n` +
        `→ Stage 6 network worker migration is viable.`,
      30000,
    );
  } else {
    new Notice(
      `❌ Worker fetch FAILED\n` +
        `error: ${result.error}\n` +
        `error type: ${result.errorName ?? "n/a"}\n` +
        `duration: ${result.durationMs} ms\n\n` +
        `→ Stage 6 falls back to "push-side SHA-first only" scope.`,
      30000,
    );
  }
}
