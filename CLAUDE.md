...
- **List-size settings** (no `[Load more]` UX) for the two GitHub-history-backed lists: `recentlyDeletedListSize` (default 30) and `fileHistoryListSize` (default 10). Eager-fetched on open via `per_page = min(N, 100)` + extra pages if `N > 100`. Users who need more open Settings and change the number (R2.7, "Performance" TBD resolution).
