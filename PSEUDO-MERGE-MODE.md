...
12. **Integration tests M/N/O/P/Q/R series**: M (branch lifecycle), N (split-push), O (resolution detection), P (multi-device), Q (recovery sweeps), R (auto-merge attempt; replaces E2/G4 for binary)
13. **Cleanup IMPLEMENTATION_PLAN.md** — прибрати все, що суперечить цьому документу (stage 2)
14. **CLAUDE.md + README.md update** — нова conflict resolution секція
