...
9. ✅ **Drain wraps**: pause ConflictWatcher → drain-start sweep → batches → drain-end sweep → finalize check → resume — Stage 9 landed; ConflictWatcher wired in main.ts (auto-start/stop) and in integration helpers; covered by `conflicts/watcher-realtime.test.ts`
10. ✅ **4-point visibility warnings** (status bar + pre-sync modal + settings + ribbon) — Stage 10 landed
...
Failure — флаки network (UND_ERR_SOCKET)

Коли інтеграційний прогон викликав real GitHub API, в одному з тестів (normalization/pull-of-bom-from-web.test.ts) 
underlying TCP-сокет до api.github.com (140.82.121.5:443) закрився посеред операції — Node-undici віддав
`SocketError: other side closed / UND_ERR_SOCKET`. 

Це транзитна мережева подія:

- GitHub Edge може закрити idle keep-alive connection чи throttle-нути коли тиск великий
- При ~104 послідовних REST-вимогах за прогон шанс одного flake-у не нульовий
- Інший прогін цього самого тесту (через 25 хв) пройшов чисто за 9.93 сек

Це не дефект ні в pull-of-bom-from-web.test.ts, ні у Stage 7c. Подібні flake-и іноді трапляються в integration suite (приклади: H2 тест навмисне симулює транзитні 503-и, J2 — 429). Наш retryUntil у GithubClient автоматично
ретрайт-ить retriable-помилки (5xx / 429 / 422); але socket-level error, який відбувається ПЕРЕД отриманням HTTP response, не завжди потрапляє в ту саму гілку, тож проявляється як throw.

**Що з цим робити довгостроково**: додати socket-error retry у retryUntil (наразі він ретраїть тільки на статус-кодах). Це окрема стадія — не блокує pseudo-merge.
