...
7. ⏳ **Split-push у processBatch** (β) + per-half marker + branch lifecycle
   - ✅ 7a: branch ops primitives + state + naming (`6dc3480`)
   - ✅ 7b: pushConflictPathsToBranch + eager-create + finalize on drain end
   - pending 7c: remove `dropPendingConflictPaths` + processBatch partition for
     edit-while-in-conflict (separate stage for clarity)
