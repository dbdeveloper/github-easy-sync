...
vault.on('delete' | 'modify' | 'rename', file):
  ...
  invalidateCounterCache()       ← debounced refresh
```
...
> **🔴 Stage 13: ця секція буде revised у Phase 2 (test audit).**
> M/N/O/P/Q/R series описана нижче — це pre-Stage-13 розкладка. Деякі
> tests аліgн з новою моделлю (e.g. classifier path-close), деякі —
> ні (e.g. drain-end sweep assertions). Phase 2 audit пройде по
> кожному test'у і поставить status `aligns_with_new_model: yes/no/partial`.
> Phase 4 видалить/перепише тести з `aligns=no` і додасть нові
> RED-tests для Stage 13 bugs (drop auto-restore, classifier row 3,
> orphan adoption, GitignoreInvariants always-write).
