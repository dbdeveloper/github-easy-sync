...
- ✅ **per-tx шлях** (`v2-mixed-recovery-spike`, 2026-06-13): змішана сесія type→resolve→type→resolve→undo×2
  →redo×1 через РЕАЛЬНУ JSON-серіалізацію → recovered doc+структура+resolution-курсор == live. Але тут кожна
  op диспатчилась як 1 транзакція з `isolateHistory` — це per-tx, НЕ доводить 1b.
- ⚠️ **1b-coalescing — НАСТУПНИЙ gate (НЕ доведено, БЛОКУЄ продакшн).** live «hello» (5 tx→1 undo-група) проти
  5 ізольованих блоків → undo на recovered відкотить 1 символ, на live — слово (**doc збігається, undo-глибина
  розходиться** → порушує тверду вимогу). Спайк має довести: writer зліплює burst→1 edit-блок (merged change),
  replay диспатчить його 1 ізольованим кроком → undo-глибина recovered == live. Реалізація — coalesce-у-writer
  (черга+flush-на-закритті-групи) АБО record-group-boundaries (per-tx блоки + прапор «нова група», replay
  ставить `isolateHistory` лише на межах); обрати в спайку. Net-глибина = (#edit − #undo + #redo) переграних.
...
емпірично вивести константи. **CPU-частина — у Web Worker** (парс NDJSON + build keep-map + emit стиснений
рядок: чисте обчислення, без Obsidian API); vault I/O + atomic-swap — на main (worker не має `vault.adapter`).
...
1. **1b-coalescing gate-спайк** (§0.5.4) — реальний burst→1 блок, replay==live undo-гранулярність. АБО свідомо
   прийняти per-tx undo й задокументувати. **Блокує продакшн.**
2. **Продакшн-екстракція**: V2 `history-log`/`history-replay` (writer §0.5.2 + replay §0.5.3 + bloat-stats
   §0.5.5) паралельними файлами (як `diff-pane-v2`), TDD; §1-`history-log.ts` помирає на Phase 6.
3. **Wiring**: writer ← DiffPane updateListener; `startSession` з V2-`joinedDocSha`; recovery-flow
   (`classifyReopen`→`reopenAction`→`ResumeRecoveryModal`→replay); `cursor.json` restore.
4. **Карусель** (§0.5.5) — окремий пізніший інкремент (worker + atomic-swap + тригери).
