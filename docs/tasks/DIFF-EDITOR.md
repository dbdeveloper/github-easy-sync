...
- **Stage 2.1 — 7-step pair-atomic commit:** §5.0 + `done.json` barrier + 11-станова
  ...
  **Carry-forward зі Stage 2.0 (review):** (1) `classifyOpen` **кидає**, якщо base/sibling
  зник між сесіями (на відміну від `readMeta`, що деградує в `null`) — open-flow має
  `try/catch`→"fresh" АБО додати 4-й статус (вирішити при wiring); (2) `reuse`-гілка має
  пере-перевірити **наявність snapshot-файлів** перед довірою (їх цілісність — §4.2);
  (3) «start over» мусить `rmdir` autosave-dir ПЕРЕД новим `startSession`, інакше stale
  `done.json`/довший `history.jsonl` переживуть свіжу сесію; (4) orphan `.sync-tmp` під
  `.diff2-autosave/` ловить лише `AtomicWriteRecovery.sweep` (Phase 11 sweep-unification) —
  не плутати з тим, що Stage 2.0 crash-тести вже покрили (вони пінять ordering, не mid-write).
