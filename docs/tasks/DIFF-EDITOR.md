...
**Cursor persistence — РАТИФІКОВАНО 2-слот ping-pong (2026-06-04, §2.9):** замість
atomic temp+rename (`rename` p95≈28ms + zero-cursor вікно) — `cursor-a.json`/`cursor-b.json`
з монотонним `seq`, пишемо стейл-слот простим `adapter.write` (≈3ms), recovery за max
валідним `seq` (НЕ за датою). Імплементується у **W3** (`cursor-store.ts` поки atomic —
позначено spec-drift marker'ом; rewrite торкне startSession + classifySweep cond-3).
...
| 3   | ~~Coalesce window cap — 10 blocks чи інше?~~ **RESOLVED (benchmark, Android 2026-06-03)** — `single-append p95 = 3.10 ms < 10 ms` → coalesce **викинуто**, append per CM6 transaction (§2.8). `QUEUE_CAP=10` лишається в `HistoryWriter` лише як contingency safety-net (production flush'ить щотранзакції). | (resolved)               |
| 4   | ~~Single DiffPane tab per conflictId enforcement?~~ **RESOLVED** — §2.4 invariant. Двa tab-и недопустимі (race на autosave). | (resolved)                                                                                          |
| 5   | Recovery dialog показує "Edits: N saved" — як рахуємо? line-count у jsonl?  (W4 — dialog ще не wired; механізм готовий: `scanHistory`/`assessHistory` дають trustworthy block-count.)                  | Так — `history.jsonl` line-count (швидкий read).                                 |
...
| 8   | ~~`joinAlgoVersion` mismatch — strict / lenient?~~ **RESOLVED (superseded)** — `joinAlgoVersion` ВИКИНУТО з meta.json; замінено на `joinedDocSha`. Mismatch тепер = classifyReopen's `library-drift` статус; strict/lenient — це обробка цього статусу в W4-діалозі (default lenient: resume на збережених snapshots, §3.1).                             | (resolved)                    |
| 9   | Cursor-timer paused коли tab not focused / Obsidian backgrounded? (Battery saving.)                                                    | Не паузимо у v1 — premature optimization; timer interval вже невеликий (2.5/6s, §2.9).            |
...
| 12  | Mid-edit vault-change banner (§5.0.f): proactive `vault.on('modify')` під час сесії → banner у DiffPane.  (v1 TOCTOU-on-`[← back]` тепер **WIRED у W1** — `classifyToctou` aborts-and-stays на зміну base/sibling від startSession.)                                            | v1: ні (тільки TOCTOU на `[← back]`). Post-v1: banner для proactive heads-up.   |
