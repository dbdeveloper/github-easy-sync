...
git-conflict з маркерами `<<<<<` / `=====` / `>>>>>`, а КОЖЕН рядок несе
**спец-префікс** (кандидати: `.`, пробіл, або unicode-маркер):
```
...
```
**Paste у НАШ редактор:** префікс авто-зрізається + маркери парсяться →
реконструкція normal+diff-strings (ver-блоки можуть бути multi-line). **Paste
деінде:** префікси лишаються — нешкідливо, користувач вичистить пізніше.
