...
**W5 §5.0.e — ДИЗАЙН готовий** (симетричне правило `[← back]`: one-side тихий single-side
write+close+log; both → save-to-alt editbox-модаль; force-overwrite прибрано — §5.0.e/§3.2.a),
**код pending**; + Step-0 committing-guard / Step-8 detach polish; далі entry-points
(file-menu, diff-ribbon, post-sync modal) + Phase 7/8/9b/10/11.
...
- **Рівно ОДНА змінилась** → **ТИХО** (без Notice — лише **log**): один `atomicWriteFile`
  `getResolved()`-вмісту у НЕзмінну сторону (змінився base → пишемо `resolved.sibling` у
  `siblingPath`; змінився sibling → дзеркально `resolved.base` у `basePath`); змінену сторону
  лишаємо «як є»; `rmdir` сесії; закриваємось у list. Конфлікт триває (нова версія зміненої
  сторони vs наш resolved незмінної) → користувач дорозв'яже потім. Той самий write, що §3.2.a
  Continue, але **close замість recreate**. НЕ через 7-step (single file → один atomic write
  достатньо, без `done.json`). Якщо після запису `SHA(base)==SHA(sibling)` — конфлікт реально
  закритий → наступний Phase A його drop'не (Step-6.5 тут НЕ дублюємо).
- **ОБИДВІ змінились** → **єдина модалка** (тут таки питаємо — незрозуміло, що узгоджували):
  > «The files you were reconciling have changed. Save your resolution under a different name,
  >  or discard?»  + **editbox** (prefilled назвою base-файлу) → `[Save]` / `[Discard]` / `[Cancel]`.
  - **[Save]** з назвою `newName`: якщо резолюція **зійшлась** (`SHA(resolved.base) ==
    SHA(resolved.sibling)`) → пишемо **ТІЛЬКИ** `newName` (один файл); якщо **частковий**
    конфлікт (`SHA` різні) → `newName` (base) **+** sibling під назвою, **похідною** від
    `newName` (`*.conflict-from-*`) → синтетична конфлікт-пара триває під новою назвою.
    Оригінали (обидва змінені ззовні) **недоторкані**. Реюзає `commit7Step` з
    `targetBasePath/targetSiblingPath` (Step-4 skip — alt-paths нові).
  - **[Discard]** → `rmdir` сесії, close (робота відкинута). **[Cancel]** → лишитись у редакторі.

**force-overwrite ВИДАЛЕНО** — ми НІКОЛИ не затираємо змінений ззовні файл. (Стара форма
§5.0.e — save-to-alt / force / cancel для будь-якого mismatch — superseded цим правилом.)
