...
- **Stage 2.0 — autosave-каталог фундамент (спільний): ✅ ЗРОБЛЕНО (2026-06-02).**
  ...
  **meta LAST** через `atomicWriteFile`+`calculateGitBlobSHA`) + `readMeta` + `classifyOpen`
  ...
  `joinAlgoVersion="diff@9.0.0"`, `joinAlgoOptions={}` (ратифіковано). Greenfield (не wired у
...
- **Stage 3 — live autosave (В ПРОЦЕСІ):** REDO-log `history.jsonl` (§2.6–§2.8) + cursor-timer (§2.9) +
  recovery dialog (§3) + cleanup/TTL (§4). «REDO» = forward-replay для crash-recovery, НЕ CM6-`redo()`.
  - **Gate-спайк (ЗРОБЛЕНО):** `tests/diff2/spikes/history-replay-structure-spike.test.ts` довів, що
    ChangeSet-only replay НЕ відновлює Rep-A structure → **format B** (structure у кожному блоці).
    §2.6 reconciled.
  - **3a (ЗРОБЛЕНО):** `src/diff2/history-log.ts` — `fnv1a32` checksum + `{seq,at,change,structure,sum}`
    serialize/parse/verify + `HistoryWriter` (queue/flush/cap + `replayDispatch` annotation). 13 тестів.
  - **3b (ДАЛІ):** replay (§3.3, через `EditorState`+`diffPaneStateField`, structure напряму з блоку) +
    cursor (§2.9 persist/read/clamp) + reopen-класифікація. **Тут же реалізувати `joinedDocSha`-gate**
    (§2.5 reconciliation-нотатка) — БЕЗ `joinAlgoVersion`-гілки.
  - **3c (ДАЛІ):** §4.2 7-умовний cleanup-sweep + done.json precedence (закриває Stage-2.0-review #2
    snapshot-integrity умова 5). Onload-ітерація = Phase 11.
...
**(0) BUG — Варіант-3 spanning replace ВТРАЧАЄ/неправильно розміщує вставлений
текст (підтверджено 2026-06-02).** Варіант-3 заміна — це **банальна текстова
заміна**: виділення з normal-рядка через diff-рядок до наступного normal-рядка,
замінене словом, має дати рівно той самий результат, ніби diff-рядка не було
(`nor[…diff…]string 2` + `TEST` → `norTESTstring 2`), і потрапити в **обидва**
файли (конфлікт зник).
- **Repro:** `tests/diff2/free-edit-resolve-bug.test.ts` (.skip — regression).
...
git-conflict з маркерами `<<<<<` / `=====` / `>>>>>`, КОЖЕН рядок несе
**спец-префікс** (кандидати: `.`, пробіл, або unicode-маркер), а ВЕСЬ payload
обгорнутий у markdown **fenced code-block** (` ``` ` зверху і знизу):
````
```
<префікс>normal string 1\n
<префікс><<<<<\n
<префікс>ver1 string 1\n
<префікс>ver1 string 2\n
<префікс>=====\n
<префікс>ver2 string 1\n
<префікс>>>>>>\n
<префікс>normal string 2\n
```
````
**Навіщо fence:** вставлене деінде (особливо в markdown) виглядає акуратним
суцільним code-блоком, а не розсипаними маркерами; а fence+префікс — це сигнатура
«наш copy», за якою редактор упевнено розпізнає payload. **Paste у НАШ редактор:**
fence + префікс зрізаються, маркери парсяться → реконструкція normal+diff-strings
(ver-блоки можуть бути multi-line). Наш редактор НЕ вставить цей блок дослівно —
зліпить з нього конфлікт. **Paste деінде:** лишається валідний code-блок із
префіксами — нешкідливо, користувач вичистить пізніше.

**Зовнішній текст з маркерами, вставлений у НАШ редактор:** теж парситься і
створює конфлікт (навіть без нашого префіксу — за наявністю `<<<<<`/`=====`/
`>>>>>` структури). «Не так весело, але допустимо — користувач сам вирішує, що
робити зі своїм текстом.» (Це коригує ранішу ідею dual-MIME + no-parse: один
текст простіший і mobile-портативний.)

**Fail-safe парсингу (строго):** якщо `<<<<<`/`=====`/`>>>>>` і/або префікс
**не знайдено саме там, де очікуються** — розбіжність хоча б на ОДИН символ →
**вставляємо УСЕ як normal strings** (жодної diff-реконструкції). Тобто парсер
fail-closed: конфлікт відновлюється лише при ІДЕАЛЬНО well-formed структурі;
будь-яке відхилення → plain normal-текст.
...
```
Вирішити: точний префікс-символ (видимість vs ризик колізії з реальним
контентом); поведінка при неповних/вкладених маркерах; чи кодувати ver==ver
(не має існувати, §1.6).
...
> **PLANNED reconciliation — `joinedDocSha` замість `joinAlgoVersion`/`joinAlgoOptions`
> (запропоновано 2026-06-02; реалізувати у Stage 3+).**
>
...
```json
{"seq":1,"at":"2026-05-29T14:32:15.103Z","change":<ChangeSet.toJSON>,"structure":<Segment[]>,"sum":"7b2a"}
```
...
3. Check existence of .diff2-autosave/<conflictId>/meta.json:
   ...
   - present → read meta.json
     ...
     - meta.joinAlgoVersion ≠ current diff version
       ...
       ── ⚠ ЗАСТАРІЛО (PLANNED, див. §2.5 `joinedDocSha`-нотатку): замінити цю
          версію-гілку на replay-validity gate
          `SHA(build(currentBase, currentSibling)) === meta.joinedDocSha`.
          Якщо НЕ збігається І входи незмінні → library-drift → start fresh
          (без діалогу — restore зі snapshot теж не відтворить). Реалізувати у 3b.
...
| ~~`joinAlgoVersion` ≠ поточна~~ **(PLANNED видалити — див. §2.5 `joinedDocSha`)** | Замість версія-проксі: gate `SHA(build(input)) === meta.joinedDocSha`. Не збігається → replay неможливий → **start fresh без modal** (входи незмінні, restore зі snapshot теж не врятує при library-drift). Жодного confirm-prompt. |
