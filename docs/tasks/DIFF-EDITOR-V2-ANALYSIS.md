...
- [~] **`src/diff2/diff-pane-v2.ts` — view-зборка (render+nav спина DONE, 4/4 happy-dom+state).** `buildDecorations`
      ...
      стане `diff-pane.ts` при wiring (старий §1 видалити). `tests/diff2/diff-pane-v2.test.ts`. **Лишилось (наступні
      інкременти):** filters auto-`\n` §2.2.4(2) + зовнішні guard-и §2.2.5 + selection-legalization §2.2.6 +
      shift+arrow; кнопки на маркерах §1.9; повна browser-валідація бандла (device-gate).
