...
- [ ] `diff-pane.ts`: StateField = RangeSet; `transactionFilter`/`changeFilter` для (a) захисту термінального `\n`,
      (b) auto-`\n` після правки (`§2.2.4(2)`), (c) зовнішніх guard-ів `§2.2.5`, (d) selection-legalization `§2.2.6`.
- [ ] `selection-rules.ts`: переписати під «selection ніколи не покидає ver-block / diff-group = атомарний символ».
- [ ] `decorations.ts` + `markers.ts`: `height:0` для прихованих term-рядків; `↵` глиф лише на ver (крім last);
      split-gradient `=====`; кольорові gutter-cells. Маркери `<<<<< / ===== / >>>>>` з кнопками.
