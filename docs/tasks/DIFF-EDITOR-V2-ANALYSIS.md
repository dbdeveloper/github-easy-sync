...
- [x] **`src/diff2/diff-line-numbers.ts` — per-side gutter §2.2.10 (4/4, pure).** `computeLineLabels` (sibling-wins:
      normal/ver2 → theirs `N`/`+N`, ver1 → base `-N`; bare-terminal рядки БЕЗ номера; EOL-less content-рядок
      нумерується) + `lineLabelsField` (cache, recompute on doc/structure change) + `diffLineNumbersGutter`
      (GutterMarker з `diff2-gutter-{side}` класами). Замінив `lineNumbers()` у `createDiffPaneState`.
      `tests/diff2/diff-line-numbers.test.ts`. Right-align (TODO #18) + кольори — CSS (view-layer).

### Фаза 4 — Резолюція + навігація
- [x] **`src/diff2/diff-resolve.ts` — резолюція scenario-2 (13/13: pure + dispatch + buttons).** `resolveGroup`
      (region-replace `[ver1.from, ver2.to)` → keep1/keep2/both/neither/join; `setStructure` дропає групу + зсуває
      решту; курсор на початок resolved-контенту TODO #9; одна транзакція = 1 undo) + `applyResolve` (+`view.focus()`
      після кліку — undo/redo доходять до CM6) + `resolveClickHandler` (mousedown на `[data-diff2-resolve]`).
      Маркери тепер несуть кнопки `[Keep↓]/[Remove↓]` / `[Apply Both]/[Remove Both]/[Join]` / `[Apply↑]/[Remove↑]`
      (§1.9). **Фікс:** `terminalProtectionFilter`+`externalGuardFilter` пропускають `setStructure`-транзакції (інакше
      блокували б видалення термінала при резолюції). join = `> `-quoted ver2 + header (label/date — Phase 6).
      `tests/diff2/diff-resolve.test.ts` + click у `diff-pane-v2.test.ts`. (DOM-клік→handler — browser-validated;
      синтетичний mousedown happy-dom не пропускає до CM6.)
- [x] Навігаційна компенсація `§2.2.4(9)` — зроблено у Фазі-3 (`cursorVert`, 1a-validated).
- [x] **Hotkeys §1.9 + bulk-toolbar (DONE, +6 tests).** `currentGroupAt` (group під кареткою) + `resolveCurrentGroup`
      + `diffResolveKeymap` (`Mod-Enter`→keep2 / `Mod-Shift-Enter`→keep1 / `Mod-Alt-Enter`→both, `Prec.highest`;
      wired). `resolveAll` (всі групи в ОДНІЙ транзакції = 1 undo; structure→[]) + `applyResolveAll` + `createBulkToolbar`
      (`[Keep all]`/`[Apply all]`/`[Join all]`). **Phase 4 ЗАВЕРШЕНО.**
- [ ] Лишилось (view-CSS, не логіка): кнопкові кольори/мітки §1.11 + capitalize (TODO #6); toolbar розміщує
      `DiffEditView` (Phase 6); join label/date wiring (Phase 6).

### Фаза 5 — Персистентність V2: COMMAND-LOG + «карусель» (КАНОН = DIFF-EDITOR.md §0.5)
> **Модель ПЕРЕРОБЛЕНА 2026-06-13** після наскрізного розбору з користувачем + 2 раунди /advisor. Стара
> «`structure` як RangeSet-серіалізація в кожному блоці / replay через `setDiffPaneState`» **ВИТІСНЕНА**.
> Повна канонічна модель — **DIFF-EDITOR.md §0.5** (стратифікована: §0.5.1 курсор DONE, §0.5.2 формат блоку,
> §0.5.3 replay, §0.5.4 gate-статус, §0.5.5 карусель design-only, §0.5.6 next-steps). Коротко:
- ✅ **Курсор резолюції — ЗБУДОВАНО** (`ba76415`): `resolveCaret`+`cursorHistory`+`cursorRestoreListener`
      (явні дані, бо CM6-native мапінг втратний). Лише резолюція; typing native. Fuzz 60/60.
- ✅ **Command-log модель — PROVEN per-tx** (`v2-mixed-recovery-spike`): `{edit|undo|redo}`, блок=мінімальна
      дельта, replay RE-RUNS commands (`undo()`/`redo()`), real JSON round-trip, doc+структура+resolution-курсор
      ==live. Витісняє §1 «REDO=forward-replay».
- ⚠️ **1b-coalescing — НАСТУПНИЙ GATE (блокує продакшн).** Тверда вимога: `history.jsonl` відтворює undo-модель
      БАЙТ-В-БАЙТ (#edit-блоків == жива undo-глибина). Спайк: burst→1 блок, replay==live undo-гранулярність.
- [ ] **Продакшн** (після 1b-gate): V2 `history-log`/`history-replay` паралельними файлами + bloat-stats;
      `cursor-store`/`cursor-timer` без змін; `splitModel`→`commit7Step` без змін; wiring.
- [ ] **Карусель** (§0.5.5, відкладено): compaction у Web Worker, тригери (undo-count OR cancelled-bytes),
      atomic-swap `.history.sync-tmp.json` маркер.
