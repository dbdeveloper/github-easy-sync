...
(PSEUDO-MERGE-MODE §7). При цьому, conflict-sibling-file буде видалено при наступному sync, так, як він повністю
...
У 2.0.1-beta (canonical: [`PSEUDO-MERGE-MODE.md`](./PSEUDO-MERGE-MODE.md) PSEUDO-MERGE-MODE §7 "Editing While in Conflict")
...
- Жодного "refuse-to-sync" guard-у не додаємо. Це б суперечило PSEUDO-MERGE-MODE §7 і ламало Scenario B (six branch commits during long
...
- Phase B "propagate live vault state to main" (SYNC2 §1):
    - якщо `Folder1/note.md` теж зник з vault (наприклад, перенесено разом із siblings) —
      side-batch містить `delete Folder1/note.md` (PSEUDO-MERGE-MODE §8 Scenario A pattern);
    - якщо `Folder1/note.md` лишився у vault (siblings перенесли окремо, base — на місці) —
      ...
      (PSEUDO-MERGE-MODE §8 Scenario E pattern). Та сама механіка, інший payload — за рахунок того, що
...
- **`!siblingExists`** (user-deleted) — Phase A дропає record, бо користувач прибрав sibling
  у file explorer (SYNC2 §1: `or drop record if sibling was deleted by user`). Diff2 R7.11
...
- **Plugin bundle semver pick** — auto-merge для `<configDir>/plugins/<id>/main.js`+`manifest.json`
  вирішує через semver-comparison (PSEUDO-MERGE-MODE §6 dispatch table, `isAtomicPluginFile`). Знову — record не
...
**Чому §4.4 (preserve-all-commits) цього не покриває.** PSEUDO-MERGE-MODE §4.4 і §7 фіксують
...
використовує у Phase A drain-pseudocode (`remove sibling if siblingSha == baseSha`) і що PSEUDO-MERGE-MODE §7
...
3. for each siblingPath, що належить цьому base-path-у (включаючи tracked- і synthetic-):
     ...
     if siblingSha == baseSha:
       ...
       (для tracked-конфлікту: ConflictStore запис НЕ чіпаємо — Phase A на
        ...
        гілку SYNC2 §1; це резервний invariant-restoring шлях)
4. close DiffPane
```
...
єдиний легальний мутатор Store-у це `evaluateConflictState` на drain-start, SYNC2 §1). Не торкається conflict-branch (не push-ить commit-и, не deleteRef-ить branch — це робота
