...
- UI підписаний на `vault.on('create', file)` (через diff2-Deleted-list layer): при матчі
  `originalPath` запис відфільтровується з рендеру (path більше не порожній). Фізично
  `.trash/<id>/` лежить на диску до наступного `[Sync with GitHub]` — для можливого manual
  recovery через filesystem, але у UI його нема. `TrashStore` сам по собі state-less щодо
  цього фільтра — UI робить vault-presence check у момент рендеру.
...
1. **`TrashStore.restore(id)` operation.** Move file back from
   ...
   per R8.1 if path occupied). Drops the trash entry; fires notify().
   Серіалізується через ту ж `serialize()` chain. Recovery sweep
   acquires a new row у R8.1 (TrashStore.restore — partial state =
...
| 4  | releasable                            | Exit protocol + proactive sibling cleanup                                           | R7.11, R7.7.c (step 2 added: SHA-compare + remove)                        | `src/diff2/exit-protocol.ts`; `diff-pane.ts` (edits)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `[←]` тепер додатково: `for siblingK of siblings(basePath): if SHA(base) == SHA(siblingK): adapter.remove(siblingK)`. Multi-sibling Scenario C працює (PSEUDO-MERGE-MODE §10). Idempotent: drain after diff2-cleaned sibling = no-op for that record. **Phase 9a interaction**: trash-watcher catches the `adapter.remove(siblingPath)` and routes the sibling bytes into `.trash/`; the trash entry has no lift shield, so layer 1b's `confirmResolved` claims it on the next drain. Add integration test to acceptance covering this round-trip                                                                                                                                                                                                                 |
